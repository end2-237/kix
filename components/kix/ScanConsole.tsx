...
export function ScanConsole({ sampleCode }: { sampleCode?: string }) {
  ...
  function validate(raw: string, method: "qr" | "code") {
    startTransition(async () => {
      ...
      if (!result.ok) {
        setFeedback({ ok: false, title: "Refusé", detail: result.error });
      ...
      if (result.kind === "token") {
        ...
        setFeedback({ ok: true, title: "Pass validé", detail: `${result.client} · ${result.event}` });
  ...
  return (
    <Card shape="panel" className="flex min-h-0 flex-col gap-4 p-5">
      ...
      <div className="relative min-h-56 grow overflow-hidden rounded-panel border border-line bg-black">
        <Image
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <button
          ...
          disabled={pending}
          className="flex h-13 items-center justify-center rounded-full bg-green px-6 text-sm font-semibold text-green-ink transition hover:brightness-105 disabled:opacity-50"
        >
          {pending ? "…" : "Valider"}
        </button>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <button
          onClick={() => (sampleCode ? validate(sampleCode, "qr") : setFeedback({ ok: false, title: "Refusé", detail: "Aucun jeton actif à scanner" }))}
          disabled={pending}
          className="glass flex h-11 items-center gap-2 rounded-full px-4 text-[13px] text-dim transition hover:bg-surface-2"
        >
          <QrIcon size={16} />
          Simuler un scan
        </button>
        {feedback ? (
          <span
            className={cn(
              "flex items-center gap-2 rounded-full px-3.5 py-2.5 text-[13px]",
              feedback.ok
