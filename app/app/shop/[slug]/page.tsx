...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      <ScreenHeader title={product.category === "vapes" ? "Vapes & puffs" : "Billard"} back="/app/shop" />

      <div className="relative h-64 overflow-hidden rounded-panel border border-line">
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-start justify-between gap-4">
          <h1 className="text-[26px] leading-7">{product.name}</h1>
          <span className="shrink-0 text-[22px] font-bold tracking-[-0.03em] text-green-text">{f(product.price)}</span>
        </div>
          ...
          <div className="-mx-5 flex gap-3 overflow-x-auto px-5 pb-1">
            {others.map((other) => (
              <Link
                ...
                className="glass w-40 shrink-0 overflow-hidden rounded-card"
              >
      ...
      <div className="fixed inset-x-0 bottom-24 z-20 mx-auto flex w-full max-w-[430px] gap-3 px-5">
